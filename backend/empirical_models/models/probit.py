...
def run_probit(df: pd.DataFrame, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(f"Probit 公式: {formula}")

    model = smf.probit(formula=formula, data=df)
    results = model.fit()

    return extract_regression_results(results, "Probit", dep_var=dep_var)
